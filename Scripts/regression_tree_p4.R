...
# Keeping only the data about P4
correlated = correlated[correlated$PROD_ID == "P4",]
...
# correlated = correlated[correlated$DATE >= "2015-05-18",]
...
# listvar = c()
listvar = c("C1_PRICE_1", "C1_PRICE_2", "C2_PRICE_1", "C2_PRICE_2", "C3_PRICE_1", "C3_PRICE_2", "MEAN_PRICE")
...
prp(dtree, type=4,fallen.leaves=FALSE, main="Regression Tree for P4", faclen=0)
...
# Write CSV in R
write.table(predicted_vector, file = "predictions/regression_tree_P4.csv",row.names=FALSE, na="",col.names=TRUE, sep=";")
