...
# Keeping only the data about P2
correlated = correlated[correlated$PROD_ID == "P2",]
...
correlated = correlated[correlated$DATE >= "2015-02-06",]
...
# listvar = c()
# listvar = c("MEDIAN_PRICE")
listvar = c("C5_PRICE_1", "C5_PRICE_2", "MEDIAN_PRICE")
# listvar = c("C5_PRICE_1", "C5_PRICE_2", "C1_PRICE_1", "C1_PRICE_2", "C3_PRICE_1", "C3_PRICE_2")
...
prp(dtree, type=4,fallen.leaves=FALSE, main="Regression Tree for P2", faclen=0)
...
# Write CSV in R
write.table(predicted_vector, file = "predictions/regression_tree_P2.csv",row.names=FALSE, na="",col.names=TRUE, sep=";")
