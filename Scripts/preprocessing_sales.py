...
# Loop to compute sales per day, mean and median of daily prices
for prod_id in split_sales.keys():
	for date in split_sales[prod_id].keys():
		sales_count = 0
		sales_mean = 0.0

		# Counting the number of sales as summing the prices in order to compute
		# the mean price
		...
		split_sales[prod_id][date] = [sales_count, sales_mean, sales_median]
